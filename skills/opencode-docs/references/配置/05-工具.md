...
工具允许 LLM 在您的代码库中执行操作。OpenCode 自带一组内置工具，您也可以通过 [自定义工具](06-自定义工具.md) 或 [MCP 服务器](../开发/04-MCP-服务器.md) 来扩展它。

默认情况下，所有工具都是 **启用** 的，且无需权限即可运行。您可以通过 [权限](02-权限.md) 来控制工具的行为。
...
要配置项目可用的 LSP 服务器，请参阅 [LSP 服务器](../开发/05-LSP-服务器.md)。
...
加载一个 [技能](../开发/02-代理技能.md)（即 `SKILL.md` 文件）并在对话中返回其内容。
...
[了解更多](../开发/04-MCP-服务器.md) 关于配置 MCP 服务器的内容。
